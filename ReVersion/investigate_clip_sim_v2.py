...
def calculate_clip_image_prompt_score(images_folder, prompt_file, prompt_name, num_images, use_custom_prompt=False):
    ...
    # naming rule of the images `{processed_prompt}/samples/{image_index:04d}.png`
    # write the results to a csv file
    # ReVersion/evaluation_massive_v2/carved_by/{prompt}/samples/{image_index}.png
    # ReVersion/evaluation_massive_v2/carved_by_tea_v2_0.2_0.5/{prompt}/samples/{image_index}.png
    results = []
    for prompt_index, prompt in enumerate(new_prompts):
        # Use the original prompt to construct the directory path (same as inference.py)
        original_prompt = prompts[prompt_index]
        processed_prompt = original_prompt.lower().replace("<r>", "<R>").format("<R>")
        
        for image_index in range(num_images):
            # Match the naming structure from inference.py: {processed_prompt}/samples/{image_index:04d}.png
            image_path = os.path.join(images_folder, processed_prompt, 'samples', f'{image_index:04d}.png')
