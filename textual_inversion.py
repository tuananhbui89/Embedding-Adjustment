...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch):
    ...
    if os.path.exists(args.validation_prompt):
        with open(args.validation_prompt, "r") as f:
            prompts = f.readlines()
        prompts = [prompt.format(args.placeholder_token) for prompt in prompts]
    else:
        prompts = [args.validation_prompt]

    for prompt in prompts:
        with torch.autocast("cuda"):
            assert args.placeholder_token in prompt, f"Placeholder token {args.placeholder_token} not in prompt {prompt}"
