...
from .tea import adjust_norm_and_slerp_3d
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def encode_prompt(
        ...
        Additional Args:
            target_prompt: the target prompt, e.g., `a photo of a man' for TEA - Test-time Embedding Adjustment Algorithm
            rho: the interpolation factor [0, 1] of the slerp
            alpha: the ratio of the norm of the vector to the target vector
        ...
        prompt = [prompt] if isinstance(prompt, str) else prompt
        target_prompt = [target_prompt] if isinstance(target_prompt, str) else target_prompt
        ...
        if prompt_embeds is None:
            ...
            # TEA - Test-time Embedding Adjustment Algorithm
            pooled_target_prompt_embeds = self._get_clip_prompt_embeds(
                prompt=target_prompt,
                device=device,
                num_images_per_prompt=num_images_per_prompt,
            )
            pooled_prompt_embeds = pooled_prompt_embeds.unsqueeze(1)
            pooled_target_prompt_embeds = pooled_target_prompt_embeds.unsqueeze(1)
            pooled_prompt_embeds = adjust_norm_and_slerp_3d(pooled_prompt_embeds, pooled_target_prompt_embeds, rho, alpha)
