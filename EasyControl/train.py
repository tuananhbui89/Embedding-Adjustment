...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to train only text_encoder_one while keeping text_encoder_two frozen.",
    )
    parser.add_argument(
        "--text_encoder_lr",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="The dimension of the LoRA update matrices for text encoders.",
    )
    parser.add_argument(
        "--text_encoder_alpha",
        type=int,
        default=64,
        help="LoRA alpha for text encoders.",
    )
    parser.add_argument(
        "--logging_dir",
        type=str,
...
def main(args):
    ...
    if args.train_text_encoder:
        ...
        # LoRA config for text encoders
        text_lora_config = LoraConfig(
            r=args.text_encoder_rank,
            lora_alpha=args.text_encoder_alpha,
        ...
        print(f"Text encoder one trainable parameters: {sum(p.numel() for p in text_encoder_one.parameters() if p.requires_grad) / 1000000:.2f}M")
        print(f"Text encoder two trainable parameters: {sum(p.numel() for p in text_encoder_two.parameters() if p.requires_grad) / 1000000:.2f}M")
    elif args.train_text_encoder_one_only:
        from peft import LoraConfig
        
        # LoRA config for text encoder one only
        text_lora_config = LoraConfig(
            r=args.text_encoder_rank,
            lora_alpha=args.text_encoder_alpha,
        ...
        # Apply LoRA to text encoder one only using add_adapter
        text_encoder_one.add_adapter(text_lora_config)
        
        print(f"Text encoder one trainable parameters: {sum(p.numel() for p in text_encoder_one.parameters() if p.requires_grad) / 1000000:.2f}M")
        print("Text encoder two: FROZEN (no LoRA applied)")
    ...
    optimizer_parameters = [transformer_parameters_with_lr]
    
    # Add text encoder parameters if training them
    if args.train_text_encoder:
        ...
        text_encoder_parameters_with_lr = {
            "params": text_encoder_one_params + text_encoder_two_params,
            "lr": args.text_encoder_lr,
        }
        optimizer_parameters.append(text_encoder_parameters_with_lr)
        
        print(f"Text encoder one parameters: {sum(p.numel() for p in text_encoder_one_params) / 1000000:.2f}M")
        print(f"Text encoder two parameters: {sum(p.numel() for p in text_encoder_two_params) / 1000000:.2f}M")
    elif args.train_text_encoder_one_only:
        text_encoder_one_params = [p for p in text_encoder_one.parameters() if p.requires_grad]
        ...
        print(f"Text encoder one parameters: {sum(p.numel() for p in text_encoder_one_params) / 1000000:.2f}M")
        print("Text encoder two parameters: 0.00M (frozen)")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        unwrapped_transformer_state = accelerator.unwrap_model(transformer).state_dict()
                        transformer_lora_state_dict = {k:unwrapped_transformer_state[k] for k in unwrapped_transformer_state.keys() if '_lora' in k}
                        save_file(
                            transformer_lora_state_dict,
                            os.path.join(save_path, "transformer_lora.safetensors")
                        )
                        
                        # Save text encoder LoRA if training them
                        if args.train_text_encoder:
                            text_encoder_one_state = accelerator.unwrap_model(text_encoder_one).state_dict()
                            text_encoder_two_state = accelerator.unwrap_model(text_encoder_two).state_dict()
                            
                            text_encoder_one_lora = {k:text_encoder_one_state[k] for k in text_encoder_one_state.keys() if 'lora' in k}
                            text_encoder_two_lora = {k:text_encoder_two_state[k] for k in text_encoder_two_state.keys() if 'lora' in k}
                            
                            save_file(
                                text_encoder_one_lora,
                                os.path.join(save_path, "text_encoder_one_lora.safetensors")
                            )
                            save_file(
                                text_encoder_two_lora,
                                os.path.join(save_path, "text_encoder_two_lora.safetensors")
                            )
                        elif args.train_text_encoder_one_only:
                            text_encoder_one_state = accelerator.unwrap_model(text_encoder_one).state_dict()
