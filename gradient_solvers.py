...
def gdquad_solv_single(vecs, lr, num_steps):
    ...
    """
    assert type(vecs) is list, "vecs must be a list"
    assert len(vecs[0].shape) == 1, "vecs must be a list of 1D vectors, but got {}".format(vecs[0].shape)
    num_vecs = len(vecs)
...
def quad_solv_for2D(vecs): 
    assert type(vecs) is list, "vecs must be a list"
    assert len(vecs[0].shape) == 1, "vecs must be a list of 1D vectors, but got {}".format(vecs[0].shape)
    assert len(vecs) == 2, "vecs must be a list of 2 vectors, but got {}".format(len(vecs))
