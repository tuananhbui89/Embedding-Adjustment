...
CUDA_VISIBLE_DEVICES=1 accelerate launch --num_processes=1 --main_process_port=29503 train_dreambooth_lora.py \
    ...
    --output_dir="outputs/cs101_db_lora_rank4_pet_cat1" \
    ...
    --validation_prompt="a photo of a sks cat wearing glasses and writing on a red notebook" \
    --validation_steps=50 \
    --rank=4 \
    --train_text_encoder
...
CUDA_VISIBLE_DEVICES=1 python generate_dreambooth.py \
    ...
    --lora_path="outputs/cs101_db_lora_rank4_pet_cat1/checkpoint-2000" \
    --output_dir="evaluation_massive/cs101_db_lora_rank4_pet_cat1/2000" \
    --prompt_file="prompts/gen_prompt_cs101_cat.csv" \
    --concept_name="sks cat" \
    --num_images=50 \
...
for custom_prompt in objectA objectB full_prompt; do
    CUDA_VISIBLE_DEVICES=0 python investigate_clip_sim_v2.py \
        --images_folder="evaluation_massive/cs101_db_lora_rank4_pet_cat1/2000/sks cat/gen_prompt_cs101_cat" \
        --prompt_file="prompts/gen_prompt_cs101_cat.csv" \
        --num_images=50 \
        --use_custom_prompt=$custom_prompt \
        --info="use_custom_prompt_$custom_prompt" \
        --sub_folder="None" \
        --output_dir="semantic_drift/cs101_db_lora_rank4_pet_cat1/2000"
done
