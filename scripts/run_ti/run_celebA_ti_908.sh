...
CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes=1 textual_inversion.py \
  ...
  --lr_warmup_steps=0 \
  --output_dir="outputs/celebA_ti_908" \
...
CUDA_VISIBLE_DEVICES=0 python generate_textual_inversion.py \
    ...
    --concept_name="<woman>" \
    --output_dir="evaluation_massive/celebA_ti_908_steps_1000" \
    --guidance_scale=7.0 \
    --embedding_path="outputs/celebA_ti_908/learned_embeds-steps-1000.bin" \
...
for custom_prompt in 'objectA' 'objectB' 'full_prompt'; do
    CUDA_VISIBLE_DEVICES=0 python investigate_clip_sim_v2.py \
        --images_folder="evaluation_massive/celebA_ti_908_steps_1000/<woman>/gen_prompt_actions_woman" \
        --prompt_file="prompts/gen_prompt_actions_woman.csv" \
        --num_images=50 \
        --use_custom_prompt=$custom_prompt \
        --info="use_custom_prompt_${custom_prompt}" \
        --sub_folder="None" \
        --output_dir="semantic_drift/celebA_ti_908_steps_1000/1000"
done
