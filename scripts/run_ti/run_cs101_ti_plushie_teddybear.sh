...
CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes=1 textual_inversion.py \
  ...
  --train_data_dir="cs101/plushie_teddybear" \
  --learnable_property="object" \
  --placeholder_token="<teddybear>" --initializer_token="teddy" \
  --resolution=512 \
  --train_batch_size=1 \
  ...
  --lr_warmup_steps=0 \
  --output_dir="outputs/cs101_ti_plushie_teddybear" \
  --validation_steps=100 \
  --save_steps=100 \
  --checkpointing_steps=10000 \
  --mixed_precision="no" \
  --validation_prompt="a photo of a <teddybear> wearing glasses and writing on a red notebook"

CUDA_VISIBLE_DEVICES=0 python generate_textual_inversion.py \
    --method="distill_ti" \
    --model_path="stable-diffusion-v1-5/stable-diffusion-v1-5" \
    --concept_name="<teddybear>" \
    --output_dir="evaluation_massive/cs101_ti_plushie_teddybear_steps_1000" \
    --guidance_scale=7.0 \
...
for custom_prompt in 'objectA' 'objectB' 'full_prompt'; do
    CUDA_VISIBLE_DEVICES=0 python investigate_clip_sim_v2.py \
        --images_folder="evaluation_massive/cs101_ti_plushie_teddybear_steps_1000/<teddybear>/gen_prompt_cs101_teddybear" \
        --prompt_file="prompts/gen_prompt_cs101_teddybear.csv" \
        --num_images=50 \
        --use_custom_prompt=$custom_prompt \
        --info="use_custom_prompt_${custom_prompt}" \
        --sub_folder="None" \
        --output_dir="semantic_drift/cs101_ti_plushie_teddybear_steps_1000/1000"
done
