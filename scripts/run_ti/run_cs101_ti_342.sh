...
CUDA_VISIBLE_DEVICES=0 accelerate launch --num_processes=1 textual_inversion.py \
  ...
  --lr_warmup_steps=0 \
  --output_dir="outputs/cs101_ti_342" \
...
CUDA_VISIBLE_DEVICES=0 python generate_textual_inversion.py \
    ...
    --concept_name="<man>" \
    --output_dir="evaluation_massive/cs101_ti_342_steps_1000" \
    --guidance_scale=7.0 \
...
for custom_prompt in 'objectA' 'objectB' 'full_prompt'; do
    CUDA_VISIBLE_DEVICES=0 python investigate_clip_sim_v2.py \
        --images_folder="evaluation_massive/cs101_ti_342_steps_1000/<man>/gen_prompt_actions" \
        --prompt_file="prompts/gen_prompt_actions.csv" \
        --num_images=50 \
        --use_custom_prompt=$custom_prompt \
        --info="use_custom_prompt_${custom_prompt}" \
        --sub_folder="None" \
        --output_dir="semantic_drift/cs101_ti_342_steps_1000/1000"
done
