...
for alpha in 0.5; do
    for rho in 0.2; do
        CUDA_VISIBLE_DEVICES=0 python generate_ti_adjust_embedding_v3.py \
            ...
            --concept_name="<cat>" \
            --output_dir="evaluation_massive/cs101_ti_pet_cat1_adjust_embedding_v3/rho${rho}_alpha${alpha}/1000" \
            --guidance_scale=7.0 \
...
for alpha in 0.5; do
    for rho in 0.2; do
        for custom_prompt in 'objectA' 'objectB' 'full_prompt'; do
                ...
                --images_folder="evaluation_massive/cs101_ti_pet_cat1_adjust_embedding_v3/rho${rho}_alpha0.5/1000/<cat>/gen_prompt_cs101_cat" \
                --prompt_file="prompts/gen_prompt_cs101_cat.csv" \
                --num_images=50 \
                --use_custom_prompt=$custom_prompt \
                --info="use_custom_prompt_${custom_prompt}" \
                --sub_folder="None" \
                --output_dir="semantic_drift/cs101_ti_pet_cat1_adjust_embedding_v3/rho${rho}_alpha${alpha}/1000"
        done
    done
done
