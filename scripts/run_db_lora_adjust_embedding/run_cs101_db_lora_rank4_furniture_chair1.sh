...
for RHO in 0.1; do
    CUDA_VISIBLE_DEVICES=1 python generate_db_adjust_embedding_v4.py \
        ...
        --output_dir="evaluation_massive/cs101_db_lora_rank4_furniture_chair1_adjust_embedding_v4/rho${RHO}_alpha0.5/2000" \
...
for RHO in 0.1; do
  for custom_prompt in objectA objectB full_prompt; do
    CUDA_VISIBLE_DEVICES=1 python investigate_clip_sim_v2.py \
        --images_folder="evaluation_massive/cs101_db_lora_rank4_furniture_chair1_adjust_embedding_v4/rho${RHO}_alpha0.5/2000/sks chair/gen_prompt_cs101_chair" \
        --prompt_file="prompts/gen_prompt_cs101_chair.csv" \
        --num_images=50 \
        --use_custom_prompt=$custom_prompt \
        --info="use_custom_prompt_$custom_prompt" \
        --sub_folder="None" \
        --output_dir="semantic_drift/cs101_db_lora_rank4_furniture_chair1_adjust_embedding_v4/rho${RHO}_alpha0.5/2000"
  done
done
