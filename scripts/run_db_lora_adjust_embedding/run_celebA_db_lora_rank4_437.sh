...
for RHO in 0.1; do
    CUDA_VISIBLE_DEVICES=1 python generate_db_adjust_embedding_v4.py \
        ...
        --lora_path="outputs/celebA_db_lora_rank4_437/checkpoint-2000" \
        --output_dir="evaluation_massive/celebA_db_lora_rank4_437_adjust_embedding_v4/rho${RHO}_alpha0.5/2000" \
...
for RHO in 0.1; do
  for custom_prompt in objectA objectB full_prompt; do
    CUDA_VISIBLE_DEVICES=1 python investigate_clip_sim_v2.py \
        --images_folder="evaluation_massive/celebA_db_lora_rank4_437_adjust_embedding_v4/rho${RHO}_alpha0.5/2000/sks man/gen_prompt_actions" \
        --prompt_file="prompts/gen_prompt_actions.csv" \
        --num_images=50 \
        --use_custom_prompt=$custom_prompt \
        --info="use_custom_prompt_$custom_prompt" \
        --sub_folder="None" \
        --output_dir="semantic_drift/celebA_db_lora_rank4_437_adjust_embedding_v4/rho${RHO}_alpha0.5/2000"
  done
done
