...
from omini.pipeline.tea import adjust_norm_and_slerp, adjust_norm_and_slerp_3d
...
def generate(
    ...
    **params: dict,
    ...
    target_prompt_embeds, target_pooled_prompt_embeds, target_text_ids = self.encode_prompt(
        ...
        prompt_2=prompt_2,
        # prompt_embeds=prompt_embeds, # REMOVE THIS
        # pooled_prompt_embeds=pooled_prompt_embeds, # REMOVE THIS
        device=device,
        num_images_per_prompt=num_images_per_prompt,
        max_sequence_length=max_sequence_length,
    )
    ...
    print("prompt_embeds", prompt_embeds.shape, target_prompt_embeds.shape)
    print("pooled_prompt_embeds", pooled_prompt_embeds.shape, target_pooled_prompt_embeds.shape)
    assert pooled_prompt_embeds.shape[0] == 1, "pooled_prompt_embeds should have batch size 1"
    assert target_pooled_prompt_embeds.shape[0] == 1, "target_pooled_prompt_embeds should have batch size 1"

    adjusted_prompt_embeds = adjust_norm_and_slerp_3d(prompt_embeds, target_prompt_embeds, rho, alpha)
    adjusted_pooled_prompt_embeds = adjust_norm_and_slerp(pooled_prompt_embeds[0], target_pooled_prompt_embeds[0], rho, alpha)
    adjusted_pooled_prompt_embeds = adjusted_pooled_prompt_embeds.unsqueeze(0)

    print("difference in norm", torch.norm(prompt_embeds - adjusted_prompt_embeds, p=2))
    print("difference in pooled_prompt_embeds", torch.norm(pooled_prompt_embeds[0] - adjusted_pooled_prompt_embeds[0], p=2))

    prompt_embeds = adjusted_prompt_embeds
    pooled_prompt_embeds = adjusted_pooled_prompt_embeds
