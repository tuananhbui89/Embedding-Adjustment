        ...
        for concept in concepts_list:
            ...
            if with_prior_preservation:
                ...
                if os.path.isdir(class_data_root):
                    ...
                    class_images_path = [x for x in class_images_path if x.is_file()]
...
def save_new_embed(text_encoder, modifier_token_id, accelerator, args, output_dir, safe_serialization=True):
    ...
    for x, y in zip(modifier_token_id, args.modifier_token):
        ...
        if safe_serialization:
            # assert ".safetensors" in output_dir, "Output directory must end with .safetensors"
            # _filename = output_dir.replace(".safetensors", f"-{y}.safetensors")
            _filename = f"{output_dir}/{y}.safetensors"
            safetensors.torch.save_file(learned_embeds_dict, _filename, metadata={"format": "pt"})
        else:
            # assert ".bin" in output_dir, "Output directory must end with .bin"
            # _filename = output_dir.replace(".bin", f"-{y}.bin")
            _filename = f"{output_dir}/{y}.bin"
            torch.save(learned_embeds_dict, _filename)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to the prompt file.",
    )
    parser.add_argument(
        "--num_images",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="1000",
        help="The step to evaluate the model.",
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                if args.modifier_token is not None:
                    if accelerator.num_processes > 1:
                        ...
                        grads_text_encoder = accelerator.unwrap_model(text_encoder).get_input_embeddings().weight.grad
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    # sanity check that model type is float32
                    assert unet.dtype == torch.float32, "Model type is not float32"
                    save_unet_path = os.path.join(args.output_dir, f"unet-steps-{global_step}")

                    unet.save_attn_procs(save_unet_path, safe_serialization=not args.no_safe_serialization)
            ...
            eval_steps = args.eval_steps.split("_")
            eval_steps = [int(step) for step in eval_steps]

            assert type(args.modifier_token) == list, f"Modifier token should be a list but got {type(args.modifier_token)}"
            concept_name = args.modifier_token[0]
            if global_step in eval_steps:
                ...
                assert args.output_images_dir != "None", f"Output images directory is not set"
                prompt_name = args.prompt_file.split("/")[-1].split(".")[0]
                output_images_dir = f"{args.output_images_dir}/{concept_name}/{prompt_name}/{global_step}"
                ...
                for i, prompt in enumerate(prompts):
                    for j in range(args.num_images):
                        print(i, j, "prompt: ", prompt.format(concept_name))
                        image = pipeline(prompt=prompt.format(concept_name), num_inference_steps=args.num_inference_steps, guidance_scale=args.guidance_scale, generator=generator).images[0]
                        image.save(f"{output_images_dir}/{i}_{j}.png")

                del pipeline
                torch.cuda.empty_cache()

                # run inference with Embedding Adjustment v4
                ...
                assert os.path.exists(args.prompt_file), f"Prompt file {args.prompt_file} does not exist"
                assert args.output_images_dir_ea != "None", f"Output images directory is not set"
                prompt_name = args.prompt_file.split("/")[-1].split(".")[0]
                output_images_dir_ea = f"{args.output_images_dir_ea}/{concept_name}/{prompt_name}/{global_step}"
                os.makedirs(output_images_dir_ea, exist_ok=True)
                ...
                for i, prompt in enumerate(prompts):
                    for j in range(args.num_images):
                        print(i, j, "prompt: ", prompt.format(concept_name))
                        image = pipeline(prompt.format(args.target_name), args.rho, args.alpha, prompt=prompt.format(concept_name), num_inference_steps=args.num_inference_steps, guidance_scale=args.guidance_scale, generator=generator).images[0]
                        image.save(f"{output_images_dir_ea}/{i}_{j}.png")
            ...
            if accelerator.is_main_process:
                ...
                if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                    ...
                    # save images
                    for i, image in enumerate(images):
                        image.save(os.path.join(args.output_dir, f"val-image-{global_step}-{i}.jpg"))
