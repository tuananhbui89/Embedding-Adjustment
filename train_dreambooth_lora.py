...
def parse_args(input_args=None):
    ...
    print("--------------------------------")
    print(args)
    assert not args.pre_compute_text_embeddings, "Our setting: pre_compute_text_embeddings=False"
    # assert not args.train_text_encoder, "Our setting: train_text_encoder=False"
    assert not args.text_encoder_use_attention_mask, "Our setting: text_encoder_use_attention_mask=False"
    print("--------------------------------")
...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                print(f"Generated {len(images)} images for class {example['index']}.")
                for i, image in enumerate(images):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    # if global_step % 10 == 0 and global_step < 200:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        ...
                        # save images
                        for i, image in enumerate(images):
                            image.save(os.path.join(args.output_dir, f"eval_{global_step}_image_{i}.png"))
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            ...
            # save images
            for i, image in enumerate(images):
                image.save(os.path.join(args.output_dir, f"epoch_{epoch}_image_{i}.png"))
